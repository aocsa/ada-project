 ...
 * @brief Defines the mapping between `utecdf::type_id` runtime type information
 * and concrete C++ types.
 *---------------------------------------------------------------------------**/
namespace utecdf {
/**---------------------------------------------------------------------------*
 * @brief Maps a C++ type to it's corresponding `utecdf::type_id`
 *
 ...
 *
 * ```
 * return utecdf::type_to_idk<int32_t>();        // Returns INT32
 * ```
 *
 * @tparam T The type to map to a `utecdf::type_id`
 *---------------------------------------------------------------------------**/
template <typename T>
...
template <utecdf::type_id t>
struct id_to_type_impl {
  using type = void;
};
/**---------------------------------------------------------------------------*
 * @brief Maps a `utecdf::type_id` to it's corresponding concrete C++ type
 *
 * Example:
 * ```
 * static_assert(std::is_same<int32_t, id_to_type<INT32>);
 * ```
 * @tparam t The `utecdf::type_id` to map
 *---------------------------------------------------------------------------**/
template <utecdf::type_id Id>
 ...
 * @param Id The `utecdf::type_id` enum
 *---------------------------------------------------------------------------**/
 ...
 * the specified `utecdf::data_type`'s `id()`.
 *
 ...
 *  }
 * };
 * utecdf::data_type t{INT32};
 * utecdf::type_dispatcher(t, size_of_functor{});  // returns 4
 * ```
 *
 * The `type_dispatcher` uses `utecdf::type_to_id<t>` to provide a default mapping
 * of `utecdf::type_id`s to dispatched C++ types. However, this mapping may be
 * customized by explicitly specifying a user-defined trait struct for the
 * `IdTypeMap`. For example, to always dispatch `int32_t`
 *
 * ```
 * template<utecdf::type_id t> struct always_int{ using type = int32_t; }
 *
 * // This will always invoke `operator()<int32_t>`
 * utecdf::type_dispatcher<always_int>(data_type, f);
 * ```
 *
 ...
 *
 * @tparam id_to_type_impl Maps a `utecdf::type_id` its dispatched C++ type
 * @tparam Functor The callable object's type
 * @tparam Ts Variadic parameter pack type
 * @param dtype The `utecdf::data_type` whose `id()` determines which template
...
template <template <utecdf::type_id> typename IdTypeMap = id_to_type_impl,
          typename Functor, typename... Ts>
constexpr decltype(auto) type_dispatcher(
    utecdf::data_type dtype, Functor f, Ts&&... args) {
  switch (dtype.id()) {
    case INT8:
